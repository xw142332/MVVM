...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.window.rootViewController = [self setTableViewRootViewController];
    [self customizeNavgationBarTheme];
    [self.window makeKeyAndVisible];
    return YES;
}
-(UIViewController *)setTableViewRootViewController{
...
-(void)customizeNavgationBarTheme{
    UINavigationBar *navgationBar = [UINavigationBar appearance];
    navgationBar.barTintColor = [UIColor colorWithRed:0.22f green:0.50f blue:0.78f alpha:1.00f];
    navgationBar.tintColor=[UIColor whiteColor];
    
    
    navgationBar.titleTextAttributes = @{
